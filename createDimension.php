...
$dimenYear = "";
	if(isset($_POST['submit'])){
		...
		if($dimenSection != "" && $dimenSection != "0" && $dimenName != "" && $dimenYear != "" && $dimenYear != "0" && $dimenGoals != "")  {
			$con = mysqli_connect(DB_HOST,DB_USER,DB_PASS,DB_NAME);
			$dimUID = $dimenYear."_DIM_".$dimenSection;
			
			$check = mysqli_query($con,"SELECT * FROM PMT_PSS_DIMENSIONS WHERE DIM_UID = '$dimUID'");
			
			if($check && $check -> num_rows == 0){
				$insert = mysqli_query($con,"INSERT INTO PMT_PSS_DIMENSIONS VALUES('$dimUID','$dimenName','$dimenGoals',$dimenYear,'$dimenSection')");
				if($insert){
					...
					$msg = "Failed to Add Dimension, please try again!!";
					$css = "danger";
				}
			}else{
				$msg = "This Dimension has already been added";
				$css = "danger";
			}
			mysqli_close($con);	
		}else{
...
<form name="form1" method="post" action="">
  ...
  <tr>
    ...
    <td><label>
      <select name="dimenSection" id="dimenSection" class="form-control" style="width:250px;">
  ...
  <tr>
    ...
    <td><label>
      <select name="dimenYear" id="dimenYear" class="form-control" style="width:250px;">
